...
import { fixedExpensesSeed, payrollSeed } from "@/lib/mock-data";
...
export default function PagosPage() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [overviewRes, boardRes, receivableRes] = await Promise.all([
  ...
  const runAction = async (shipmentId: number | undefined, action: "collect" | "settle" | "driver-paid") => {
    if (!shipmentId) {
  ...
  const kpis = useMemo(
    () => [
      { label: "Total por cobrar", value: formatCOP(overview.totals.total_receivable), color: "text-purple-600" },
      { label: "Por liquidar", value: formatCOP(overview.cod.pending), color: "text-pending" },
      { label: "Liquidado oficina", value: formatCOP(overview.cod.settled), color: "text-delivered" },
    ],
    [overview]
  );
        ...
        <>
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-4">
            ...
            {receivable.length === 0 ? (
              ...
              <div className="mt-3 space-y-2">
                {[...receivable].sort((a, b) => b.days_oldest_debt - a.days_oldest_debt).map((item) => (
                  <div key={item.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-slate-200 p-3">
                    <div>
                      <p className="font-semibold text-slate-900">{item.company || item.name}</p>
                      <p className="text-xs text-slate-500">{item.phone} · {item.owed_shipments_count} envíos</p>
                      <span className={`mt-1 inline-flex rounded-full px-2 py-1 text-xs font-semibold ${debtTone(item.days_oldest_debt)}`}>{item.days_oldest_debt} días</span>
                    </div>
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-4">
            ...
            <div className="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              {board.map((item) => (
                <article key={item.driver_id} className="rounded-lg border border-slate-200 p-3">
                  <p className="font-semibold text-slate-900">{item.name}</p>
                  <p className="mt-2 text-sm">COD pendiente: <strong>{formatCOP(item.cod_pending)}</strong></p>
                  <p className="text-sm">COD recaudado: <strong>{formatCOP(item.cod_collected)}</strong></p>
                  <p className="text-sm">Pendiente pago: <strong>{formatCOP(item.unpaid_fees)}</strong></p>
                  <p className="text-sm">Entregas hoy: <strong>{item.today_deliveries}</strong></p>
                  <div className="mt-3 flex flex-wrap gap-2">
                    <button onClick={() => runAction(item.shipment_id, "collect")} className="rounded border border-slate-300 px-2 py-1 text-xs">Marcar recaudo</button>
                    <button onClick={() => runAction(item.shipment_id, "settle")} className="rounded border border-slate-300 px-2 py-1 text-xs">Liquidar</button>
                    <button onClick={() => runAction(item.shipment_id, "driver-paid")} className="rounded border border-slate-300 px-2 py-1 text-xs">Pago conductor</button>
                  </div>
                </article>
              ))}
            </div>
          ...
          <section className="grid gap-4 lg:grid-cols-2">
            <article className="rounded-xl border border-slate-200 bg-white p-4">
              <h2 className="text-base font-semibold text-slate-900">Gastos fijos (mock)</h2>
              <div className="mt-3 space-y-2">
                {fixedExpensesSeed.map((expense) => (
                  <div key={expense.name} className="rounded-lg border border-slate-200 p-3">
                    ...
                    <p className="text-sm text-slate-600">{formatCOP(expense.amount)} · vence {expense.dueDate}</p>
                  </div>
                ))}
              </div>
            </article>
            <article className="rounded-xl border border-slate-200 bg-white p-4">
              <h2 className="text-base font-semibold text-slate-900">Nómina (mock)</h2>
              <div className="mt-3 space-y-2">
                {payrollSeed.map((employee) => (
                  <div key={employee.name} className="rounded-lg border border-slate-200 p-3">
                    ...
                    <p className="text-sm text-slate-600">{formatCOP(employee.salary)} · {employee.frequency}</p>
                  </div>
                ))}
              </div>
            </article>
