...
import { PortalAccessButton, PortalStatusBadge, usePortalAccessList, type PortalAccessStatus } from "@/components/portal-access-panel";
import {
  Badge,
  Button,
  Card,
...
export default function ClientesPage() {
  ...
  const portal = usePortalAccessList();
  const setPortalStatus = (clientId: number) => (status: PortalAccessStatus) =>
    portal.setRows((current) => current.map((row) => row.client.id === clientId ? { ...row, status } : row));
  ...
  return (
    <div className="animate-fade-in space-y-5">
      <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        ...
        <div className="flex w-full flex-col gap-2 md:w-auto md:flex-row">{portal.canManage ? <Button variant="secondary" onClick={() => router.push("/clientes/portal")} className="w-full md:w-auto">Accesos al portal</Button> : null}<Button onClick={openCreate} className="w-full md:w-auto">Nuevo cliente</Button></div>
      </header>
