...
### Added
- nuevo endpoint `POST /api/shipments/address-preview` para previsualizar direcci�n, sugerencias geogr�ficas, zona inferida y coordenadas antes de crear el pedido.
- lista de coincidencias seleccionables dentro del modal `Nuevo pedido`.
- mapa previo del punto resuelto dentro del flujo de creaci�n del env�o.

### Changed
- el formulario de pedidos se reorden� por bloques operativos:
  - cliente remitente;
  - destinatario + tel�fono;
  - ciudad;
  - captura guiada/manual de direcci�n;
  - preview de direcci�n + mapa;
  - zona;
  - cobros.
- `Ciudad de entrega` ahora funciona como selector principal y la zona queda como ajuste posterior asistido.
- al guardar un pedido, el frontend env�a `recipient_lat` y `recipient_lng` cuando el operador ya eligi� una coincidencia geogr�fica.

### Quality
- nueva regresi�n backend para `POST /api/shipments/address-preview`.
- validado con:
